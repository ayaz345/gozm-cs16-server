...
if((isset($_GET["bid"]) AND is_numeric($_GET["bid"])) OR (isset($_GET["bhid"]) AND is_numeric($_GET["bhid"]))) {
	...
	if(mysql_num_rows($resource) == 0) {
		...
		$ban_info = array(
			...
			"player_id"	=> $player_id,
			"player_comid" => GetFriendId($player_id),
			"player_ip"	=> $player_ip,
			"ban_start"	=> $ban_start,
	...
	if(isset($_GET["bid"])) {
		...
		while($result = mysql_fetch_object($resource)) {
            ...
            $reason = convert_cp1251_to_utf8($result->ban_reason);
...
if($config->display_demo == "enabled"){
	if(isset($_GET["bid"])) {
		$bid = $_GET['bid'];
		$resourcecc	= mysql_query("SELECT id, bid, demo, comment FROM $config->amxdemos WHERE bid = '".mysql_real_escape_string($bid)."'") or die(mysql_error());
	}else{
		$bhid = $_GET["bhid"];
		$resourcecc	= mysql_query("SELECT id, bid, demo, comment FROM $config->amxdemos WHERE bhid = '".mysql_real_escape_string($bhid)."'") or die(mysql_error());
	}
	$demos	= array();
	...
	while($resultss = mysql_fetch_object($resourcecc)) {
		...
		if(isset($_GET['bid'])) {
			$resourcec	= mysql_query("SELECT id, name, comment, email, addr, date FROM $config->amxcomments WHERE bid =".mysql_real_escape_string($bid)." ORDER BY date ASC") or die(mysql_error());
		}else{
			$bhid = $_GET['bhid'];
			$resourcec	= mysql_query("SELECT id, name, comment, email, addr, date FROM $config->amxcomments WHERE bhid =".mysql_real_escape_string($bhid)." ORDER BY date ASC") or die(mysql_error());
		}
		$ban_comments	= array();
		$i=0;
...
if ((isset($_POST['action'])) && ($_POST['action'] == "insert")) {
	...
	if(isset($_GET['bid'])) {
			$sql = mysql_query("SELECT date FROM $config->amxcomments WHERE addr = '".$user_ip."' AND bid = '".$bid."' ORDER BY date DESC LIMIT 0, 1");
	}else{
		$bhid = $_GET["bhid"];
	...
	if ($count > 0 && $time < $anti_flood)
	{
		...
		if(isset($_GET["bid"]) ){
			...
			echo "<meta http-equiv=\"refresh\" content=\"".$delay.";url=\"ban_details.php?bhid=$bid\">";
		}
		exit();
	} else {
		$name = trim($name);
