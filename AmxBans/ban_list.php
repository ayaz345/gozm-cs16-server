...
$resource	= mysql_query("SELECT bid, player_id, player_ip, player_nick, admin_nick, ban_reason, ban_created, ban_length, server_ip, server_name, map_name, se.gametype, aa.nickname FROM $config->bans AS ba LEFT JOIN $config->servers AS se ON ba.server_ip=se.address LEFT JOIN $config->amxadmins AS aa ON aa.nickname=ba.admin_nick ORDER BY ban_created DESC LIMIT ".$query_start.",".$query_end) or die(mysql_error());
