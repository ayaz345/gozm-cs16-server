...
display_mutemenu(id, pos) 
{
  	...
  	for(new a = start; a < end; ++a)
	{
		...
		if( i == id || get_user_flags(i) & ADMIN_LEVEL_H  && !(get_user_flags(i) & ADMIN_RCON))
		{
			++b
...
{ 
	...
	if ((get_user_flags(VictimID) & ADMIN_LEVEL_H) && VictimID != id)  
		return PLUGIN_HANDLED;
//	if (get_user_flags(VictimID) & ADMIN_RCON)
//		return PLUGIN_HANDLED;
		
	new s_Flags[4],VictimName[32],flags
//	,Float:f_GagTime
...
*/
	...
	colored_print(0,"^x04*** ^x03 %s^x01 has been muted",VictimName) 
...
public CMD_UnGagPlayer(id,level,cid)   /// Removed gagged player ( done via console command )
{
	...
	if (id != 0)
		colored_print(0,"^x04*** ^x03 VIP %s^x01 let^x03 %s^x01 speak",AdminName,VictimName)
