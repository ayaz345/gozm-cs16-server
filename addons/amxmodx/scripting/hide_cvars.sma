...
{
	new s_Line[64], i_LineCount, i_LineLen, i_Index

	while (read_file(s_FilePath, i_LineCount++, s_Line, charsmax(s_Line), i_LineLen))
	{
		if (i_LineLen && (!equal(s_Line, "//", 2)))
		{
			g_Cvars[i_Index] = s_Line
			i_Index++
		}
	}
