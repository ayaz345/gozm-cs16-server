...
public checkVotes(id)
{
    ...
    for (i = 0; i < g_voteSelectedNum[id]; ++i)
        if (g_voteCount[a] < g_voteCount[i])
            a = i

    static votesNum, iRatio, iResult
    votesNum = g_voteCount[0] + g_voteCount[1] + g_voteCount[2] + g_voteCount[3] + g_voteCount[4]
    iRatio = votesNum ? floatround(get_cvar_float("amx_votemap_ratio") * float(votesNum), floatround_ceil) : 1
    iResult = g_voteCount[a]
    ...
    if (g_choosed != -1)
    {
        ...
        {
            static menuBody[512], tempMap[32], len
            ArrayGetString(g_mapName, g_choosed, tempMap, charsmax(tempMap))
            len = formatex(menuBody, charsmax(menuBody),
                             g_coloredMenus ? "\y%L: \w%s^n^n" : "%L: %s^n^n", id, "THE_WINNER", tempMap)
...
{
    static a
    for (a = 0; a < g_voteSelectedNum[id]; ++a)
...
displayVoteMapsMenu(id, pos)
{
    ...
    static menuBody[512], tempMap[32], keys
    static a, b, c, start, end, len
    ...
    static menuName[64]
    formatex(menuName, charsmax(menuName), "%L", "en", "VOTEMAP_MENU")
...
public actionVoteMapMenu(id, key)
{
    ...
    switch (key)
    {
        ...
        {
            static Float:voting
            voting = get_cvar_float("amx_last_voting")
            ...
            static Float:vote_time
            vote_time = get_cvar_float("amx_vote_time") + 2.0
            ...
            static menuBody[512], players[32], pnum, keys, len
            ...
            {
                len = formatex(menuBody, charsmax(menuBody), g_coloredMenus ? "\y%L^n\w^n" : "%L^n^n", id, "WHICH_MAP")

                static c
                for (c = 0; c < g_voteSelectedNum[id]; ++c)
                {
                    ArrayGetString(g_mapName, g_voteSelected[id][c], tempMap, charsmax(tempMap))
                ...
                len = format(menuBody, charsmax(menuBody),
                             g_coloredMenus ? "\y%L^n%s?^n\w^n1. %L^n2. %L^n" : "%L^n%s?^n^n1. %L^n2. %L^n",
                             id, "CHANGE_MAP_TO", tempMap, id, "YES", id, "NO")
                keys = MENU_KEY_1|MENU_KEY_2
            }

            static menuName[64]
            formatex(menuName, charsmax(menuName), "%L", "en", "WHICH_MAP")
            ...
            static tempMapA[32], tempMapB[32], tempMapC[32], tempMapD[32]
...
public actionMapsMenu(id, key)
{
    switch (key)
    {
        ...
        default:
        {
            static a, _modName[10]
            a = g_menuPosition[id] * 8 + key
...
displayMapsMenu(id, pos)
{
    ...
    static menuName[64]
    formatex(menuName, charsmax(menuName), "%L", "en", "CHANGLE_MENU")
