...
clientDisplayMenu(id, pos)
{
    ...
    len = formatex(menuBody, charsmax(menuBody),
                     g_coloredMenus ? "\yAMX Mod X Client Menu\R%d/%d^n\w^n" : "AMX Mod X Client Menu %d/%d^n^n" ,
                     pos + 1, (g_clientMenusNumber / MENUITEMSPERPAGE) + (((g_clientMenusNumber % MENUITEMSPERPAGE) > 0) ? 1 : 0))
...
public addmenuitem_cmd(id, level, cid)
{
    ...
    static menuBody[STRINGSIZE], menuCmd[STRINGSIZE], flags[STRINGSIZE], menuAccess, menuPlugin[STRINGSIZE]
