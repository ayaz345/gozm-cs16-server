...
new Float:g_prevDistance[MAX_PLAYERS]
...
handle_time_elapsed(id, eventType)
{
	...
	if (elapsedSeconds >= maxSeconds)
	{
		...
		switch (eventType)
		{
			case EVENT_JOIN:
			{
				formatex(msgReason, 63, " Не выбрал команду за %d секунд", maxSeconds)
				formatex(msgAnnounce, 63, "was kicked for failing to choose a team within %ds", maxSeconds)
			}
			case EVENT_SPEC:
			{
				formatex(msgReason, 63, " Был в спектрах более %d секунд", maxSeconds)
				formatex(msgAnnounce, 63, "was kicked for spectating longer than %ds", maxSeconds)
			}
			case EVENT_AFK:
			{
				formatex(msgReason, 63, " Был AFK более %d секунд", maxSeconds)
				formatex(msgAnnounce, 63, "was kicked for being AFK longer than %ds", maxSeconds)
			}
		}
		...
		// log the kick
		new logText[128]
		format(logText, 127, "%s", msgAnnounce)
		trim(logText)
		new name[32]
