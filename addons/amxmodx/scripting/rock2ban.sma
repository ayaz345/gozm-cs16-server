...
public menu_handle(id, menu, item)
{
	...
	{
		colored_print(id, "^x04%s^x01 Выбранный игрок вышел", g_prefix)
		return PLUGIN_HANDLED
	}
	...
	else
	{
		...
		if (CHECK_FLAG(g_targets[id], target))
		{
			...
			log_amx("%s %s removed vote from %s (%d/%d of %d)", g_prefix, voter_name, target_name, g_votes_for[target], max_votes, get_playersnum())
		...
		log_amx("%s %s set vote for %s (%d/%d of %d)", g_prefix, voter_name, target_name, g_votes_for[target], max_votes, get_playersnum())
		colored_print(id, "^x04%s^x01 Ты проголосовал за бан^x03 %s", g_prefix, target_name)
...
public check_votes(target)
{
	...
	if (g_votes_for[target] >= max_votes)
	{
		...
		log_amx("%s %s is banned by %d/%d of %d votes", g_prefix, name, g_votes_for[target], max_votes, get_playersnum())
...
{
	new user_id = get_user_userid(id)
	new ban_time = get_pcvar_num(pcvar_bantime)
	...
	if (announce)
	{
		...
		colored_print(0, "^x04%s^x01 Игрок^x04 %s^x01 забанен общим голосованием", g_prefix, name)
	}
}
