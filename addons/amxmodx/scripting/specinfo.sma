...
#define KEYS_STR_LEN 63

//cvar pointers
new p_red, p_grn, p_blu;
...
public server_frame( )
{
    ...
    {
        id = players[i];
        new user_buttons = get_user_button(id)
        
        if( user_buttons & IN_FORWARD )
            cl_keys[id] |= IN_FORWARD;
        if( user_buttons & IN_BACK )
            cl_keys[id] |= IN_BACK;
        if( user_buttons & IN_MOVELEFT )
            cl_keys[id] |= IN_MOVELEFT;
        if( user_buttons & IN_MOVERIGHT )
            cl_keys[id] |= IN_MOVERIGHT;
        if( user_buttons & IN_DUCK )
            cl_keys[id] |= IN_DUCK;
        if( user_buttons & IN_JUMP )
            cl_keys[id] |= IN_JUMP;
    }
...
public clmsg(id)
{
    ...
    format( msg, KEYS_STR_LEN, "%s%s", msg, keys_string[id2][1] );
    format( msg, KEYS_STR_LEN, msg,
