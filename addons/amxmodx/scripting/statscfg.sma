...
public cmdCfg(id, level, cid)
{
    ...
    else if (equali(cmds, "list"))
    {
        ...
        formatex(lVariable, charsmax(lVariable), "%L", id, "VARIABLE")
        ...
        if (start != -1)
        {
            ...
            {
                formatex(lOnOff, charsmax(lOnOff), "%L", id, get_xvar_num(g_menuDataId[a]) ? "ON" : "OFF")
...
displayCfgMenu(id, pos)
{
    ...
    new len = formatex(menu_body, charsmax(menu_body),
...
saveSettings(filename[])
{
    ...
    fputs(file, ";Generated by Stats Configuration Plugin. Do not modify!^n;Variable  Description")
    ...
    for (new a = 0; a < g_menuDataNum; ++a)
    {
        if (get_xvar_num(g_menuDataId[a]))
        {
            if (equal(g_menuData[a], "ST_", 3))
            {
                formatex(text, charsmax(text), "%-24.23s ;%L", g_menuDataVar[a], LANG_SERVER, g_menuData[a])
            }
            else
            {
                formatex(text, charsmax(text), "%-24.23s ;%s", g_menuDataVar[a], g_menuData[a])
            }
            fputs(file, text)
        }
    }
