...
public ConsoleCommand_WeaponMaxClip(id, lvl, cid)
{
    ...
    {
        static szWeaponName[17], iId
        copy(szWeaponName, charsmax(szWeaponName), "weapon_")
        ...
        if (iId && ~NOCLIP_WPN_BS & 1<<iId)
        {
            ...
            static iMaxClip, bool:bIsShotGun
            iMaxClip = str_to_num(szMaxClip)
            bIsShotGun = !!(SHOTGUNS_BS & (1<<iId))
            ...
            if (iMaxClip && iMaxClip != g_iDftMaxClip[iId])
            {
                ...
                {
                    static iShotGunType
                    iShotGunType = iId == CSW_M3 ? m3 : xm1014
