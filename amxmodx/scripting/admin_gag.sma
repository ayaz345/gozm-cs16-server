...
{ 
	...
	register_concmd("amx_gag","CMD_GagPlayer",ADMIN_IMMUNITY,"<nick or #userid> <a|b|c> <time>") 
	register_concmd("amx_ungag","CMD_UnGagPlayer",ADMIN_IMMUNITY,"<nick or #userid>") 
} 
...
#if GagReason == 1
			colored_print(id,"^x04////// ^x01Тебя заткнули по причине: %s ^x04//////",gs_GagReason[id]) 
#else
			colored_print(id,"^x04////// ^x01Тебя заткнули! ^x04//////") 
#endif
...
{ 
	...
	{ 
#if GagReason == 1
		case 2:   colored_print(0,"^x04////// ^x03VIP %s ^x01заткнул ^x03%s ^x01на ^x04%0.0f ^x01минут. ^x04//////",AdminName,VictimName,(f_GagTime / 60)) // debug added
   		case 1:   colored_print(0,"^x04////// ^x03VIP %s ^x01заткнул ^x03%s ^x01на ^x04%0.0f ^x01минут. ^x04//////",AdminName,VictimName,(f_GagTime / 60)) 
#else
		case 2:   colored_print(0,"^x04////// ^x03VIP %s ^x01заткнул ^x03%s ^x01на ^x04%0.0f ^x01минут. ^x04//////",AdminName,VictimName,(f_GagTime / 60)) // debug added
   		case 1:   colored_print(0,"^x04////// ^x03VIP %s ^x01заткнул ^x03%s ^x01на ^x04%0.0f ^x01минут. ^x04//////",AdminName,VictimName,(f_GagTime / 60)) 
#endif
...
public CMD_UnGagPlayer(id,level,cid)   /// Removed gagged player ( done via console command )
{
	...
	switch(get_cvar_num("amx_show_activity"))   
	{ 
   		case 2:   colored_print(0,"^x04////// ^x03VIP %s ^x01позволил общаться ^x03%s ^x04//////",AdminName,VictimName) 
   		case 1:   colored_print(0,"^x04////// ^x03VIP %s ^x01позволил общаться ^x03%s ^x04//////",AdminName,VictimName)
  	}
...
{ 
	if(g_GagPlayers[id]) // Checks if disconnected player is gagged, and removes flags from his id.
	{
		...
		colored_print(0,"^x04////// ^x01Игрок с молчанкой ^x03%s ^x01обиделся и отсоединился. ^x04//////",Nick)
...
public client_infochanged(id)
{
	if(g_GagPlayers[id])
	{
		...
		{
			colored_print(id,"^x04////// ^x01Игроки с молчанкой не могут менять свои ники! ^x04//////")
...
public task_UnGagPlayer(TaskParm[])	// This function is called when the task expires
{
	...
	colored_print(0,"^x04////// ^x03%s ^x01снова может общаться. ^x04//////",VictimName)
