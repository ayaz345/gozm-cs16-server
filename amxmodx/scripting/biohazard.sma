...
new bool:g_zombie[25], bool:g_falling[25], bool:g_disconnected[25], bool:g_blockmodel[25], 
    ...
    g_modelent[25], g_menuposition[25], g_player_class[25], g_player_weapons[25][2]
...
//	register_logevent("team_win_round", 2, "1=Terrorists_Win")
//	register_logevent("team_win_round", 2, "1=CTs_Win")
...
public client_putinserver(id)
{
	...
	new param[1]
	param[0] = id 
	set_task( 10.0 , "recordDemo" , id , param , 1 )
}

public recordDemo(param[])
{
	client_cmd( param[0],"stop")
	client_cmd( param[0],"record GO_ZOMBIE")
}
...
check_round(leaving_player)
{
	...
	if (g_preinfect[leaving_player] && !g_gamestarted)
	{
		...
		g_preinfect[id] = true
		colored_print(id, "^x04 ***^x01 Preinfected zombie leaves.")
		colored_print(id, "^x01 [BIOSCAN] Result: You're ^x03INFECTED^x01!")
...
/*	for GIRLS
	if(!(g_zombie[id]) && get_user_flags(id) & ADMIN_LEVEL_E)
	{
...
{
	set_dhudmessage(60, 50, _, 0.445, 0.89, 0, _, 0.2, 0.2)
...
//		colored_print(0, "^x04 id = %d", id)
//		colored_print(0, "^x04 last_zombie = %d", last_zombie)
...
//			colored_print(id, "^x01Пиши ^x04/guns ^x01в чат, чтобы перевыбрать оружие.")
...
stock fm_get_weapon_id(id)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return 0;
	
	return get_pdata_int(id, OFFSET_WEAPONTYPE, EXTRAOFFSET_WEAPONS);
}

stock fm_get_weapon_ammo(id, ARMOR)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return 0;
	
	return get_pdata_int(id, OFFSET_CLIPAMMO, EXTRAOFFSET_WEAPONS);
}
...
stock fm_reset_user_primary(id)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return;
...
stock fm_lastprimary(id)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return 0;
	
	return get_pdata_cbase(id, OFFSET_LASTPRIM);
}

stock fm_lastsecondry(id)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return 0;
	
	return get_pdata_cbase(id, OFFSET_LASTSEC);
}

stock fm_lastknife(id)
{
	// Prevent server crash if entity is not safe for pdata retrieval
	if (pev_valid(id) != 2)
		return 0;
	
	return get_pdata_cbase(id, OFFSET_LASTKNI);
}