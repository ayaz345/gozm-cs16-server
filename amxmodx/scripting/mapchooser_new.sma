...
public plugin_init( )
{
	...
	pcv_vote_delay		= pcvar_register( "amx_vote_delay",		"2"    )
	pcv_votemap_ratio	= pcvar_register( "amx_votemap_ratio",	"0.60" )
	pcv_extendmap_max 	= pcvar_register( "amx_extendmap_max",	"40"   )
...
public checkVotes( )
{
	...
	if( g_rtv )
	{
		if( pcv_lastround && get_pcvar_num( pcv_lastround ) )
		{
			...
			say_hudmessage( 0, 210, 0, 0, 0.45, 0.21, 1, 20.0, 10.0, 0.5, 0.15, 4, "%L", AUTO_LANG, "FINAL_ROUND" )
...
public initiateLastRound( )
{
	...
	say_hudmessage( 0, 100, 200, 0, 0.45, 0.21, 1, 20.0, 10.0, 0.5, 0.15, 4, "%L", AUTO_LANG, "FINAL_COMING" )
