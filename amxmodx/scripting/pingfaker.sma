/*================================================================================
	...
	* Fix this to send a single SVC_PINGS message, using the real
	   arguments used by the HL engine, which can be found here:
	   http://wiki.amxmodx.org/Half-Life_1_Engine_Messages#SVC_PINGS
...
new g_maxplayers, g_connected[33], g_isbot[33], g_offset[33][2], g_argping[33][3]
new g_loaded_counter, g_pingoverride[33] = { -1, ... }
new Array:g_loaded_authid, Array:g_loaded_ping

public plugin_init()
{
	register_plugin("Ping Faker", "1.5", "MeRcyLeZZ")
	
	cvar_enable = register_cvar("pingfake_enable", "1")
	cvar_ping = register_cvar("pingfake_ping", "73")
	cvar_flux = register_cvar("pingfake_flux", "10")
	...
	cvar_fileonly = register_cvar("pingfake_fileonly", "1")
	...
	for (player = 1; player <= g_maxplayers; player++)
	{
		...
		if (g_pingoverride[player] < 0)
		{
			if (get_pcvar_float(cvar_multiplier) > 0.0)
			{
				...
				ping = clamp(floatround(ping * get_pcvar_float(cvar_multiplier)), 0, 4095)
			}
			else
				ping = clamp(get_pcvar_num(cvar_ping) + random_num(-abs(get_pcvar_num(cvar_flux)), abs(get_pcvar_num(cvar_flux))), 0, 4095)
		}
		else
