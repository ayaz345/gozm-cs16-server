...
public ShowMostDisruptive   = 1 // shows rounds most disruptive player
...
public plugin_init()
{
	...
	register_clcmd("say /top", "cmdTop15", 0, "- display top 15 players (MOTD)")
	...
	register_clcmd("say_team /top", "cmdTop15", 0, "- display top 15 players (MOTD)")
...
get_kill_info(id, iKiller, sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	if (iKiller && iKiller != id)
	{
		...
		iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%L^n", id, "DID_DMG_HITS", izAStats[STATS_DAMAGE], g_izKilled[id][KILLED_KILLER_HEALTH])
		iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%L^n", id, "YOU_DID_DMG", izVStats[STATS_DAMAGE])
	}
...
//		add(sBuffer, MAX_BUFFER_LENGTH, t_sText)
		colored_print(0, "^x04***^x01 Best Player: [^x03     %s^x01     ]   ->   [^x04     %d dmg^x01     ]", g_izUserRndName[id], iMaxDamage)
	}
...
format_kill_ainfo(id, iKiller, sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	if (iKiller && iKiller != id)
	{
		...
		iLen = format(sBuffer, MAX_BUFFER_LENGTH, "%L (%dхп, %dбронь) >>", id, "KILLED_BY_WITH", t_sName, t_sWpn, distance(g_izUserAttackerDistance[id]), 
...
format_top15(sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	new lKills[16], lDmg[16]
//	, lEff[16]
	
	format(lKills, 15, "Kills")
	format(lDmg, 15, "Damage")
//	format(lEff, 15, "Eff%")
	
	iLen = format(sBuffer, MAX_BUFFER_LENGTH, "<body bgcolor=#E9E8E2><font color=#000000><pre>")
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2s %31s   %6s    %6s^n", "#", "Nick", lKills, lDmg)

	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body bgcolor=#FFFFFF><font color=#FF0000><pre>")
	get_stats(0, izStats, izBody, t_sName, MAX_NAME_LENGTH)
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2d %31s %6d      %6d^n", 1, t_sName, izStats[STATS_KILLS], izStats[STATS_DAMAGE])
						
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body bgcolor=#FFFFFF><font color=#008000><pre>")
	get_stats(1, izStats, izBody, t_sName, MAX_NAME_LENGTH)
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2d %31s %6d      %6d^n", 2, t_sName, izStats[STATS_KILLS], izStats[STATS_DAMAGE])

	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body bgcolor=#FFFFFF><font color=#0000FF><pre>")
	get_stats(2, izStats, izBody, t_sName, MAX_NAME_LENGTH)
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2d %31s %6d      %6d^n", 3, t_sName, izStats[STATS_KILLS], izStats[STATS_DAMAGE])									
						
	iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "<body bgcolor=#FFFFFF><font color=#000000><pre>")
	for (new i = 3; i < iMax && MAX_BUFFER_LENGTH - iLen > 0; i++)
	{
		get_stats(i, izStats, izBody, t_sName, MAX_NAME_LENGTH)
		replace_all(t_sName, MAX_NAME_LENGTH, "<", "[")
		replace_all(t_sName, MAX_NAME_LENGTH, ">", "]")
		iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "%2d %31s %6d      %6d^n", i+1, t_sName, izStats[STATS_KILLS], izStats[STATS_DAMAGE])
	}
}
...
//	fAcc = accuracy(izStats)
	...
	client_print(id, print_chat, "* %L", id, "YOUR_RANK_IS", iRankPos, iRankMax, izStats[STATS_KILLS], izStats[STATS_DAMAGE])
