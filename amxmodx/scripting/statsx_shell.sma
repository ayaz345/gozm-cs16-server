...
//		add(sBuffer, MAX_BUFFER_LENGTH, t_sText)
		colored_print(0, "^x04***^x01 Best Player:^x04  %s^x01  ->  [^x03  %d dmg^x01  ]", g_izUserRndName[id], iMaxDamage)
	}
...
format_kill_ainfo(id, iKiller, sBuffer[MAX_BUFFER_LENGTH + 1])
{
	...
	if (iKiller && iKiller != id)
	{
		...
		iLen = format(sBuffer, MAX_BUFFER_LENGTH, "^x04 ***^x01 Killed by^x04 %s^x01 (%d hp)", t_sName, g_izKilled[id][KILLED_KILLER_HEALTH])
		
//		if (!izStats[STATS_HITS])
//			iLen += format(sBuffer[iLen], MAX_BUFFER_LENGTH - iLen, "^x04 ***^x01 No hits...")
	}
	else
		format(sBuffer, MAX_BUFFER_LENGTH, "^x04 ***^x01 You have no killer.")
...
/*
	...
	iLen = format(sBuffer, MAX_BUFFER_LENGTH, "^x04 ***^x01 Last result:^x04 %d^x01 hits and^x04 %d^x01 damage", izStats[STATS_HITS], izStats[STATS_DAMAGE])
...
public cmdMe(id)
{
	...
	{
//		colorChat(id, CHATCOLOR_RED, "%L", id, "DISABLED_MSG")
		colored_print(id, "^x04 ***^x01 You have^x04 NO HITS")
		return PLUGIN_HANDLED
	}
...
public cmdRank(id)
{
	...
	{
//		colorChat(id, CHATCOLOR_RED, "%L", id, "DISABLED_MSG")
		colored_print(id, "^x04 ***^x01 Isn't ranked")
		return PLUGIN_HANDLED
	}
...
//	colorChat(id, CHATCOLOR_GREEN, "* %L", id, "YOUR_RANK_IS", iRankPos, iRankMax, izStats[STATS_KILLS], izStats[STATS_HITS], ((effec(izStats) + accuracy(izStats)) / 2))
	colored_print(id, "^x04 ***^x01 Your rank is^x04 %d^x01 of^x04 %d^x01 with^x03 %d^x01 kills and^x03 %0.0f%%^x01 skill", iRankPos, iRankMax, izStats[STATS_KILLS], ((effec(izStats) + accuracy(izStats)) / 2))
