...
public ShowPlayerMenu(id)
{
	...
	static menubody[512]	
  	new len = format(menubody, 511, "Кого забаним? ^n^n")
  	...
  	for(new a = start; a < end; ++a)
	{
  	...
  	if(end != gi_TotalPlayers) 
	{
    		format(menubody[len], 511 - len, "^n9. %s...^n0. %s", "Еще", gi_MenuPosition ? "Назад" : "Выход")
    		keys |= MENU_KEY_9
  	}
  	else
		format(menubody[len], 511-len, "^n0. %s", gi_MenuPosition ? "Назад" : "Выход")
...
public ChooseMenu(id, key)
{
  switch(key)
  {
    ...
    default:
    {
		...
		if (containi( invul[gi_Sellection], ga_PlayerAuthID[gi_Sellection] ) != -1)  
		{
...
public run_vote()
{
  log_amx("Voteban starter %s against %s %s", ga_PlayerName[gi_VoteStarter], ga_PlayerName[gi_Sellection], ga_PlayerAuthID[gi_Sellection])
  format(ga_MenuData,(MAX_menudata-1),"Ban \r%s \wfor \y%d \wminutes?^n1. Yup!^n2. No!!!",ga_PlayerName[gi_Sellection], get_pcvar_num(gi_BanTime))
  ga_Choice[0] = 0
  ga_Choice[1] = 0
...
public outcom()
{
	...
	if( ga_Choice[0] > ga_Choice[1] )
	{
		...
		log_amx("[AMXX] BANNED: %s, STARTER: %s", ga_PlayerName[gi_Sellection], ga_PlayerName[gi_VoteStarter])
	...
	else
	{
		...
		invul[gi_Sellection] = ga_PlayerAuthID[gi_Sellection]
	}
	return 0
}
